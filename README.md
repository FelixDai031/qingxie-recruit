...
仓库里不含任何云环境凭据，需要自己填两项才能跑起来：

| 文件 | 改哪个字段 | 去哪里找值 |
|---|---|---|
| `config/env.js` | `envId` | 微信开发者工具 → 云开发 → 环境概览 → **环境 ID** |
| `project.config.json` | `appid` | 微信公众平台 → 开发管理 → 开发设置 → **AppID** |

`config/env.js` 里默认是占位符 `YOUR_CLOUD_ENV_ID`，这是刻意为之 —— 仓库公开后不会泄露原作者的云环境。改完再按下面「快速开始」部署云函数。
...
2. 开通后记录你的 **环境 ID**。
3. 打开 `config/env.js`，把 `envId` 的占位值 `YOUR_CLOUD_ENV_ID` 改成你的环境 ID。

   > 云环境 ID 统一放在 `config/env.js`，不要在 `app.js` 里写死。
   > 这个文件在仓库里只保存占位符模板，你本地改成真实值后 git 不会把它提交上去。
